...
df_features=pd.read_csv(input, sep='\t', na_filter = False)
df_clause['Extended']=[]
...
for index, row in df_clause:
    ...
    for c in cl_list:
        if '#' not in c:
            ext_cl.append(df_features.loc[df_features['fnum'] == int(c)])
        else:
             ext_cl.append('#'+df_features.loc[df_features['fnum'] == int(c.replace('#',''))])
